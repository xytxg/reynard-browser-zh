...
当前维护代码已同步原作者仓库截至 `7ea1cd9` 的更新，包括 iOS 密钥串密码/一次性验证码自动
填充，以及 iOS 15 及更高版本的初步自动画中画支持。合并后额外增加了安全降级：可选的
UIKit/Gecko 动态接口缺失、Gecko 视图尚未创建或画中画候选层瞬时消失时不会终止浏览器；
画中画协调器释放时也会停止轮询并解除代理。iOS 13 和 iOS 14 不创建画中画协调器，原有浏览
能力保持不变。
...
成功产物会创建 `build-<运行编号>` 预发行版；推送 `v*` 标签时则创建同名正式 Release。
